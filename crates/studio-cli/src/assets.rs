...
const DOCTRINE: [(&str, &str); 15] = [
    ...
    (
        ...
        include_str!("../assets/doctrine/consistency/character-sheet.md"),
    ),
    (
        "audio/design.md",
...
const SKILLS: [SkillDoc; 10] = [
    ...
    SkillDoc {
        ...
        duties: &[
            ...
            "**每张卡出多个视图**，不是一张大头照：角色走转身图五视图，\
             场景走建立/主机位/反打/细节，道具走正/侧/使用/比例。\
             必需视图缺一个提交就会被挡下，各类必需哪些见 \
             consistency/character-sheet.md。",
            "每张卡写一段 identity_prompt，**一次写定**，**这张卡的**所有视图\
             逐字复用——每个视图的 prompt 都以它开头，一个字不改，后面再接\
             该视图特有的机位／取景。\
             **那半句要写成人话，不是把视图的枚举名抄上去**——抄枚举名的结果是\
             七个视图长得一模一样，真发生过，控制面会在提交时挡下（V14）。\
             **而且取景要写「什么占满画面」，不要写「裁到哪」**：\
             「双手和瓶身占据画面」有效，「肩部以上入画」无效——后者实测完全\
             不生效，出来还是全身照。告诉模型画面里该有什么，比告诉它裁到哪\
             可靠得多，跟「排版禁止项要写成正向约束」是同一条道理。\
             连带一条：**特写类视图不要写「不裁切」**，它们按定义就是裁切的。\
             **每张卡的身份锁写的是这张卡自己那个东西**：角色卡写那个人，\
             场景卡写那个空间，道具卡写那个道具。\
             把角色的身份锁抄进场景卡或道具卡，出来的三张卡是同一个人——\
             真发生过，控制面会在提交时就挡下（V13）。\
             只有角色卡这一段要逐字包含分镜定下的身份锁。",
            "标出主视图（每张卡有且仅有一个）并把它排在 views 第一位——**顺序就是\
             生成顺序**。其余视图的 derived_from 是一个列表，第一项永远是主视图。\
             并行出八张，出来的是八个长得像但不是同一个人的角色。\
             **后面挂什么，按构图分家**：全身类视图累积挂前面已定稿的全身类视图；\
             特写类视图（face_close / expressions / hands_props / wardrobe_detail /\
             场景的 detail）**只挂主视图加前面已定稿的特写类视图**，\
             不要把全身照挂给它们。参考图会连构图一起锁——挂四到七张全身照，\
             出来的特写就还是全身照，提示词写得再准也压不住（真发生过）。",
            "写明一致性锁定：角色外观、机位签名、环境、排版禁止项。",
            "降级策略写死：核心系列不可用就结构化阻塞，不自动换系列。",
        ],
        notes: &[
            "这是 hybrid 阶段：你定资产计划，**提交通过之后控制面立刻开始生成**，\
             生成完才上确认门——门上要人看的是卡片本身，不是一份没见过的 JSON。\
             所以提交完 waiting_on 会变成 system：那时候等着，隔一会儿再 \
             studio.status，别在那里收尾。",
            "status 一律写 planned，path 与 provenance 留空——\
             那两个字段由控制面回填，不是你写的。",
            "卡片是**测量用的参考素材**，不是好看的剧照。\
             **角色卡与道具卡**走中性灰底、均匀柔光、无阴影投射——主体要能\
             从背景里孤立出来才好比对。\
             **场景卡不套这条**：那个空间本身就是主体，孤立不了。\
             它走自然光照与真实环境，同一空间同一陈设，变的只有机位与光线时段；\
             建立镜头放在灰底上就没有全貌可看了。\
             戏剧性打光留给成片。",
        ],
    },

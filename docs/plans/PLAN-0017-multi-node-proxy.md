...
## S4 真机验收 ✅（2026-09-05，结论有保留）

集群：8 × A800 80GB PCIe（79 GiB/台），经 `comfy.i-yongqi.xyz`。

`a_saturated_queue_is_waited_out_not_reported_as_failure`：24 镜
（640×384、22 帧）并发提交。

| 结果 | |
|---|---|
| 提交成功 | **24 / 24** |
| 提交耗时 | 中位 509 ms、最慢 974 ms，全部在 1.0 秒内返回 |
| 出片 | **24 / 24**，总耗时 177.7 秒 |

**没验到的那一半要说清楚：这一轮没有压出排队。** 代理在 24 并发上不阻塞
`/prompt`，直接派发下去了，所以「排队被等过去」那条路径真机没走到，只有
单元测试覆盖（脚本桩连回两次 503 再回 200）。测试里写死了这个判据：压不出
排队时打印警告说明本轮没验到，不让一次没压满的运行冒充通过。

**连带更正 SPEC §2.1**：初稿写「八台节点 × 队列深度 16，必然被踩到」，
实测否掉了。这一步做的是按代理文档写明的契约做的**防御**，不是对已观测
故障的修复。

`doctor` 在真集群上的输出（S3 那条）：
`8 个节点：cuda:0 NVIDIA A800 80GB PCIe × 8（每台 79 GiB）`。

顺带重跑 `real_comfy` 全套——这次改动碰了提交与轮询两条主路径。

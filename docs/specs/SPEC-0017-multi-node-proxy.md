...
> **这条初稿写的是「八台节点 × 队列深度默认 16，必然被踩到」。实测把它否掉了。**
>
> 2026-09-05 S4 真机：24 个提交并发压进 8 台节点，**全部在 1.0 秒内返回**
> （中位 509 ms、最慢 974 ms），一个都没排队。代理在这个量级上不阻塞
> `/prompt`——它把请求直接派发下去，排队发生在各节点自己的 ComfyUI 队列里，
> 那是提交之后的事，不占我们这个 HTTP 请求。
>
> 所以这一节改的东西是**按代理文档写明的契约做的防御，不是对已观测故障的
> 修复**。要真触发那条路径，得压到代理确实派发不出去为止，而那个阈值在
> 24 之上——继续往上试的代价是等量的 GPU 时间，不值得为验一条防御性分支花。
>
> 记在这里是因为：**「必然被踩到」这种话本来就该由实测来说，不该由推理来说。**
...
| 真机（烧 GPU） | 24 镜并发提交，一个都不失败，且全部出片 |

**最后一项验到了哪一层，要说清楚。** 2026-09-05 实测：24 个提交全成功、
24 镜全部出片（178 秒）；但**没有压出排队**（最慢的提交 974 ms），所以
「排队被等过去」这一半只有单元测试覆盖，真机没验到。这条测试里写死了这个
判据——压不出排队时它打印一行警告说明本轮没验到，而不是让一次没压满的运行
冒充通过。

...
API 格式里它是**平铺的点号兄弟键**——`<槽位名>.<prefix><序号>`，序号从 1 起：

```jsonc
"ref_images.ref_image_1": ["load_ref1", 0],
"ref_images.ref_image_2": ["load_ref2", 0]
```

> **⚠ 更正（2026-09-05）。** 这一节原来写的是嵌套对象
> `"ref_images": {"ref_image_1": [...]}`，依据是「挂 2 张图提交，图校验通过」。
> **那个依据不足，结论是错的。** 嵌套形态图校验能过、图能跑、有产出，但
> ComfyUI 的执行器**不把它当连线**——加载节点成了死节点，参考一个都没进模型。
> 于是从 PR #19 合并到这次更正之间，整条参考通道都是空转的。
>
> 对拍才发现：同一 seed、同一提示词，不挂参考图 / 挂纯绿 / 挂纯红，
> 三份输出逐字节相同；换成点号形态后绿红两份画面立刻不同。
>
> 判据是**把加载节点指向一个不存在的文件**：连线被认出来，节点就是活的，
> 图校验会拒；认不出来就是死节点，校验照过。六种候选形态里只有点号那种被拒。
>
> 这跟 `COMFY_DYNAMICCOMBO_V3` 是同一套编码（见 SPEC-0015 §2.4），也是同一类
> 教训：**「图校验通过」能挡住的错误比想象的少**。
...
V9 是实现时补的：§4.1 那个例子自己就写着
`{ "at_frame": 0, "kind": "clip", "asset_id": "S02.tail22" }`——**镜头之间接得住
靠的就是引用上一镜的尾段**，而那东西不可能出现在 `visual_assets` 里，它要等
S02 渲完才存在。所以引用有两个来源：登记过的资产走 V7，镜间片段走 V9。
两者按形状区分：后缀是 `tail` / `head`（后面可跟帧数）的算镜间引用，

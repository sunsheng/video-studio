...
> §6 单开一章处理它：多视图规格，以及一条不可降级的一致性硬要求。
> OpenAI 那条路评估后不采用，理由与实测记在 §6.2.2。
>
> ⚠️ **§6 的后端选型已被 issue #12 推翻，那一章的 Z-Image 相关内容作废，
> 重写待实测后进行。** 详见 §6 开头的说明。
...
> **本章已按实测重写（2026-09-05）。** 后端从 Z-Image 单参考换成
> **FLUX.2 dev 累积锁定**，机制从「一律以主视图为参考」换成「挂上已定稿的
> 全部视图」。规格与实现见
> [SPEC-0016](specs/SPEC-0016-visual-assets-executor.md) /
> [PLAN-0016](plans/PLAN-0016-visual-assets-executor.md)，已落地。
>
> 换的理由不是「换个更新的模型」，恰恰是 §6.2.1 那条没变的原则：
> 单参考是「一致性不可降级」这条原则下的次优解。原文里那些**没被推翻的东西
> 原样留着**——§6.2.2 的 OpenAI 实测（能力面矩阵、「文字锁不住脸」那组对照）
> 是这一整章的地基，跟换不换后端无关。
...
2. **控制面生成**：主视图先出，其余视图**挂上已定稿的全部视图**累积锁定生成，
   落盘并登记产物。
...
> 实现时这一点改动了 `StageKind::Hybrid` 的语义：原来写的是「确认后由控制面
> 执行生成」，现在是「**控制面先执行，再把产物送上门**」。顺序反过来才符合
> 第 3 点——门要人看的是卡片，不是一份没见过的 JSON。

而贯穿三步的第一原则是 **§6.2.1 的一致性硬要求：达不到就阻塞，不降级、不妥协。**

### 6.2 后端：ComfyUI + FLUX.2 dev

图像后端只有一条：**ComfyUI + FLUX.2 [dev]**（Black Forest Labs，
`flux2_dev_fp8mixed` + `mistral_3_small_flux2_bf16` + `flux2-vae`），
同时提供两种生成方式，两种都必须支持：

| 方式 | 用途 | 实现 |
|---|---|---|
| 文生图 | 出主视图（角色 `front_full`、场景 `establishing`） | `flux2_dev/t2i`，20 步，约 57 秒 |
| 多参考编辑 | 出其余所有视图，挂上已定稿的全部视图 | `flux2_dev/multiref_edit`，1–10 张参考 |

走的是与视频渲染同一条 ComfyUI HTTP 通道，尺寸自由、本机免费。
接线细节（`ReferenceLatent` 串链、两处必须偏离官方模板的地方）在
SPEC-0016 §3。

OpenAI 那条路评估过，**不采用**，两条独立的理由记在 §6.2.2。

#### 6.2.0 为什么是 FLUX.2，不是 Z-Image，也不是视频抽帧

这一节记的是选型的实测依据，不是偏好。**同一个身份提示词、同一套卡片规格
（768×1344、中性灰底、匀光、全身入画）**，三条路真机跑过：

| | 主视图耗时 | 多视图一致性 | 构图一致性 | 覆盖范围 |
|---|---|---|---|---|
| **FLUX.2 dev** | 57 s | ✅ 累积锁定，最多 10 张参考 | ✅ **四视图全身入画，一张没漂** | 任意视图 |
| Z-Image Turbo | 15 s | 未验：多参考走 `TextEncodeZImageOmni`，配的是 Z-Image **Base**，权重不在这台机器上 | — | — |
| MiniMax 转身抽帧 | 168 s / 4 视图 | ✅ 同一次采样，物理上同一个人 | ✅ 机位固定 | ❌ 只覆盖连续运动扫得到的角度 |
| MiniMax 逐视图独立采样 | 22 s（5 帧） | ✅ 参考锁得住 | ❌ **会自己裁成中景**，提示词写了 `full body head to toe` 也没守住 | 任意视图 |

两条结论：

1. **构图一致性是这一项的胜负手。** 卡片是测量用的参考素材——一张全身、
   一张中景，喂进 R2V 时比例就不对了。画面好不好看在这里是次要的。
2. **「视频抽帧」这条路当初是被逼出来的，不是选出来的。** MiniMax H3 三份
   基线全是 `CreateVideo → SaveVideo`，没有静态图出口，用它出卡片在物理上
   只能渲一小段再抽帧。2026-09-05 重探这台机器，FLUX.2 dev 的权重齐了，
   这个约束就不成立了，所以路线跟着改。

**累积锁定的实测**（出第 N 个视图时把主视图 + 已定稿的 1..N-1 全挂上）：
...
**阴性对照要如实记**：同 seed 同提示词、一张参考都不挂，出来是个「相像但
不同」的人——脸型更圆、领口从方领变圆领、鞋子多了红条。参考确实在起作用，
但在提示词写得很死、seed 又相同的情况下，**边际影响是中等而不是压倒性的**。
更硬的机械证据是耗时随参考数单调上升（57 → 64 → 102 → 138 s），参考 latent
确实进了 conditioning。

> 之所以要同时给视觉证据和机械证据、并写明效应量级：上一次判断「参考生效」
> 栽过——AUTOGROW 槽位写成嵌套对象时接线是死的，图校验过、图能跑、有产出，
> 三样全成立，全部既有测试都没感觉。经过见 §6.6。
...
- 手段是**参考图累积锚定**：主视图先出，其余视图挂上**已定稿的全部视图**
  生成（§6.3）。**纯文字锚定不算达标**，§6.2.2 有实测证据。
...
- 探不到 ComfyUI 的机器做不完这个阶段，只能做到 Agent 提交 `asset_plan` 为止；
  卡片本身的验收得在能连上 ComfyUI 的机器上做。
- 没有健康的 ComfyUI 节点时，这个阶段以 `comfy_unavailable` 结构化阻塞，
  **不降级、不跳过、不用纯文字凑合出图**——这与 §6.2.1 是同一条原则。

这是明知的取舍：**一致性优先于「到处都能跑」**。

> 原文这里写的是「开发环境（无 GPU、无 ComfyUI）」。**那个前提已经不成立**：
> 云端会话可以配好 `COMFY_NODE` 指向一台真实的 ComfyUI，本项目就是这么做
> 卡片验收的。所以这一条的正确说法是「探不到就做不完」，能力面是探出来的，
> 不是按环境名写死的。
...
```
COMFY_NODE             图像与视频共用同一个入口；入口不可达时视觉资产阶段直接阻塞
COMFY_TOKEN            入口要鉴权时的 Bearer token
```

基线名不做成环境变量：`flux2_dev/t2i` 与 `flux2_dev/multiref_edit` 是
`config/models.toml` 里那份固定契约的一部分，能被环境变量换掉就等于开了
一条静默降级的口子。
...
- 生成顺序是**累积锁定**，一张卡内严格串行：
  ...
  1. **先出主视图**，单独出、无参考：角色是 `front_full`，场景是 `establishing`。
     走 `flux2_dev/t2i`。
  2. **其余视图挂上已定稿的全部视图**生成，走 `flux2_dev/multiref_edit`：
     提示词 = 逐字相同的 `identity_prompt` + 该视图特有的机位/表情描述，
     参考图 = 主视图 + 前面每一个已经 ready 的视图（最多 10 张，
     `MAX_CARD_REFERENCES`）。文字负责说清「要什么角度」，参考图负责保证
     「是同一个人」——而且是**同时与所有已定稿视图自洽**，漂移无处可去。
  3. 任一视图与主视图明显不符 → 重生成该视图，而不是接受漂移。
     重生成到顶仍不达标 → 整阶段阻塞（§6.2.1），不放行。

- 主视图**先出、单独出**是硬要求：并行生成八个视图，出来的是八个长得像
  但不是同一个人的角色。这一条与后端无关，是生成模型的固有性质。
  所以**卡与卡之间可以并发，一张卡内部必须串行**——实现就是这么排的。
- **参考少一张就报错，不许悄悄退回无参考的那条路。** 退回去出来的是个
  「长得像但不是同一个人」，而且一路绿到交付。实现上，非锚点视图拿不到
  它 `derived_from` 里的任何一张，这个视图直接判 `failed`。
- **部分成功不算成功。** 只要有一个视图没出来，或者出来了却传不回 ComfyUI
  当不了后面的锚点，整阶段阻塞（§6.8）。`studio.retry_stage` 会跳过已经
  `ready` 的，只补没成的那几个。
...
FLUX.2 走 ComfyUI，画幅按参数设，要多少是多少。**卡片和首帧图的画幅要求
本来就不同**，分开处理：

| 用途 | 画幅 | 说明 |
|---|---|---|
| 角色卡 | 竖构图，短边 768（`CARD_SHORT_EDGE`），按 `aspect` 推长边并取整到 16 | 全身入画不裁切 |
| 场景卡 | 横构图或与成片同画幅 | |
| 道具卡 | 方构图 | 带比例参照 |
...
**不新增 crate，也没有新抽象。** 图像后端走的是已有的 `studio-comfy` 通道，
`studio-openai` 当前不建——它只在 §6.2.2 那条路被启用时才需要。

```
studio-pipeline  Pipeline::visual_assets
                 ├── generate_card(卡)    一张卡：主视图先出，其余累积挂参考
                 ├── generate_view(视图)  提交一张图并等出片
                 └── upload_view(视图)    把出来的图传回 ComfyUI 好当下一张的参考

                 依赖 core + engine + comfy + media。
```

初稿在这里设计了一个 `trait ImageBackend { generate, edit }`，**实现时没有落地**，
理由记在这里免得下次又想加：这条链路只有一个后端，两个方法的差别就是
「挂不挂参考图」，而挂参考图的接线（`ReferenceLatent` 串链）本来就长在基线
JSON 里、不在 Rust 里——抽出一个只有一个实现、又表达不了真正差异的 trait，
换来的是一层要维护的间接，换不来任何东西。§6.2.2 那条路真被启用时再抽，
那时才知道接口该长什么样。

**参考图的可变数量走的是 `ReferenceChain`**（`studio-pipeline::workflow`），
它是 #14 片段库的第三种可变槽位形状：不是 AUTOGROW 的平铺点号编号，
而是把 `LoadImage → VAEEncode → ReferenceLatent` 逐张串成链，
conditioning 一环扣一环。节点 id 用 `ref{n}_{短名}`，确定性生成，
同一份计划两次组装出来的图逐字节相同。
...
```jsonc
{
  "asset_plan": {
    ...
    "core_model_family": "minimax_h3",      // 视频系列，与卡片后端是两套模型
        ...
        "views": [
          {
            ...
            "derived_from": ["front_full", "three_quarter"],
                                           // 非主视图必填：已定稿的全部视图，
                                           // 锚点排第一，只能引用排在自己前面的，
                                           // 最多 10 个（§6.3、schema V10–V12）
            "status": "ready",             // planned/generating/ready/failed
            "path": "media/assets/C01/front_full.png",   // 控制面回填，相对路径
            "provenance": {                // 控制面回填，可审计
              "backend": "flux2_dev/multiref_edit",   // 实际用的基线
              "width": 768, "height": 1360,
              "seed": 40201,               // 由 (asset_id, view) 定，不随机——
                                           // retry_stage 必须能复现同一张
              "references": ["front_full", "three_quarter"]
            }
          }
        ]
      }
...
- `derived_from` 是**一致性的执行依据，不只是元数据**：非锚点视图必须列出
  已定稿的全部视图，控制面照着它去取参考图喂给 `multiref_edit`。
  没写就没有锚点可用，是 `schema_violation`。三条校验在提交那一刻就报：

  | 规则 | 内容 |
  |---|---|
  | V10 | 锚点视图必须排在 `views` 的第一位 |
  | V11 | `derived_from` 第一项必须是锚点，其余只能是**排在自己前面**的视图 |
  | V12 | `derived_from` 最多 10 项（`MAX_CARD_REFERENCES`，模型上限） |

  V11 的「只能引用前面的」不是洁癖：生成是串行的，引用后面的那张在轮到自己时
  还不存在，等于没有参考——而没有参考出来的是长得像但不是同一个人。
...
1. **上传**：ComfyUI 走 `/upload/image` 把卡片送上节点，拿到服务端文件名。
   接入方式收敛成单个 `COMFY_NODE`（那一侧通常是代理，多节点分发由它负责），
   所以不再需要「按节点分别上传」。
2. **绑定**：`minimax_h3` 的 R2V 有四个可变槽位——`ref_images` / `ref_videos` /
   `ref_video_audios` / `ref_audios`，类型 `COMFY_AUTOGROW_V3`。四个都已真机核验。
3. **核验**：按现有规矩，`bindings_verified` 只能在**真机跑通并且人眼看过画面**
   之后置 true。图校验通过只够写「接线已验证」。
4. **选型**：优先 r2v（全程锚点）而不是 i2v（只喂第一帧，往后放身份会漂）——
   这是调研里最明确的一条一致性结论，要写进 prompt skill 的 doctrine。

> **这里踩过一个必须记住的坑。** AUTOGROW 槽位一度写成嵌套对象
> `{"ref_images": {"ref_image_1": [...]}}`——ComfyUI 的图校验**过**、图**能跑**、
> **有产出**，三样全成立，所有既有测试都是绿的，但执行器不认这个形状，
> 加载节点是**死节点**，参考从来没进过模型。整条参考通道空转了三个 PR。
>
> 正确写法是平铺的点号兄弟键：`"ref_images.ref_image_1": [...]`。
>
> 留下两条方法：**（a）** 判断一条参考通道通没通，用阳性对照——换一张参考，
> 输出必须跟着变；只看「跑完了、有产出」等于没看。**（b）** 不烧 GPU 的探法：
> 把加载节点指向一个不存在的文件，图校验若报错说明这条线是活的，若通过说明
> 校验器根本没看见它。
...
| 对象 | 改动 |
|---|---|
| `visual` skill | 从「写一份计划」改为「定身份锁 + 视图清单 + 逐视图提示词」；补多视图规格、主视图先行的生成顺序、`derived_from` 要列出已定稿的全部视图；说明门后面是**图**不是 JSON |
...
| doctor | 增加视觉资产体检：`flux2_dev` 的两份基线在不在、核验没核验、ComfyUI 节点是否健康 |
| `config/models.toml` | 增加 `[flux2_dev]` 契约段（模型文件名、禁用变体） |
| 新基线 | `assets/workflows/flux2_dev/t2i.json`、`flux2_dev/multiref_edit.json` |

**Agent 不需要知道用哪个模型出图。** 上面这些改动里没有一条是「告诉 Agent 用
FLUX.2」——不出 `models/flux2_dev.md` 能力卡，`asset_plan.backend` 填的是传输
通道（`comfyui`）而不是模型名。卡片模型是控制面的实现细节，Agent 给的是身份锁
和视图清单，控制面负责把它变成图。这与硬规则 1（Agent 看不见二进制名和命令行）
是同一条思路。

（顺带修掉一处会误导的残留：`asset_plan.strategy` 的示例原本写着
「先出开发片段再抽帧」——那正是已经废弃的视频抽帧路线。示例改成「这批卡怎么排」，
并明说用什么模型出图不用 Agent 写。）
...
| `flux2_dev` 基线缺失或未核验 | `model_contract_violation`，remedy 指向补齐并真机核验基线 |
| 单张视图生成失败 | 该 view 标 `failed` 并记原因；同卡后面的继续跑（它们照样能拿更前面的当参考），**收尾时整阶段阻塞** |
| 视图出了图，但传不回 ComfyUI | 也算不达标。它自己交付得了，却当不了后面的锚点，下一轮 retry 还会卡在同一处 |
| 部分视图 ready、部分 failed | **不放行**，抛 `comfy_failed`，detail 逐条列出哪个视图、为什么 |
...
设计的；现在只有一条路，`comfy_unavailable` / `comfy_failed` /
`model_contract_violation` 已经够用，语义也更准。将来 §6.2.2 那条路启用时再看
要不要加。

#### 两条在真实 Codex 会话里才暴露的缺陷（2026-09-05）

单元测试全绿、真机验收也过了，但一跑完整的十阶段会话，这两条立刻现形。
记在这里是因为它们的**形状**比它们本身更值得记。

**一、超时值压在实测耗时的中位数上。**
上传参考图当时跟 `/prompt`、`/history` 共用一个 30 秒读超时。而实测：一张
1.09 MB 的卡片图经当时那条代理传上去要 **25–38 秒**（事后得知那条代理当时
正有已知故障，同一张图在故障期外量到过 14.5 秒——**这几个数字不是干净的
基线**，但结论不依赖它们：几 MB 的传输和几百字节的 JSON 凭什么共用一个
上限）。于是锚点视图图出来了、
落盘了、`status` 是 `ready`，只是传不回去，后面五个派生视图全部报
「参考图还没生成出来」——**一句与事实相反的话**。

修法有两半，缺一不可：

- 大块传输（上传参考图、下载产物）用独立的读超时（5 分钟），
  不跟控制面的小 JSON 共用一个值。
- **失败原因要分开记。**「没生成出来」和「生成了但传不回去」排查方向完全相反，
  而下游看到的现象是同一个「缺参考图」。现在每个视图记下自己当不了参考的
  真实原因，下游的报错原样带上它。

**二、部分失败被放行。** §6.8 这张表早就写着「部分 ready、部分 failed → 不放行」，
实现里却只在 `ready == 0` 时才拦——六个视图成一个，照样上确认门，
`_gate_choice` 记着 `approve`。**规格写了不等于实现了**，尤其是这种「只在
坏路径上才走到」的分支，正路径的测试永远碰不到它。

共同的教训跟 AUTOGROW 那次是同一个：**「机器说成功了」不等于「我要的事发生了」。**
断言要测效果，不要测「跑完了没有」。
...
> 下面逐条标了状态，但**这里不是待办清单**——它记的是「这次重构为什么这么排」。
> 还没做完的事在 [TODO.md](TODO.md)，那份有「做完就删」的纪律，两处各记一份
> 迟早对不上。要找活干看那边。

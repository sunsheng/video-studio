...
> ⚠️ **这一章的后端选型已经作废，等实测后重写。**
>
> 本章写的是 **ComfyUI + Z-Image 单后端**，机制是「主视图先出、其余视图
> 一律以主视图为参考图生成」。issue #12 推翻了它，理由是**参考图容量**：
>
> - Z-Image-Edit 是**单参考**——出第 5 个视图时只能挂主视图，前 4 个已定稿
>   视图的信息用不上。
> - FLUX.2 [dev] 一次吃**最多 10 张参考图**——出第 5 个视图时把主视图加前 4 个
>   一起挂上，新视图必须同时与它们自洽。这叫**累积锁定**，漂移无处可去。
>
> **§6.2.1「一致性不可降级」那条原则不变，恰恰是按这条原则才要换**——
> 单参考是这条原则下的次优解。Z-Image 已从仓库移除（issue #12 定的）。
>
> 还有一路本章没有的：MiniMax i2v 从定稿卡片出 1–2 秒锚视频，给 R2V 的
> 3 个 video reference 槽位提供内容，带的是图片参考给不了的运动先验。
>
> **仍然有效的部分**：§6.1 的三步目标、§6.2.1 的一致性硬要求、§6.2.2 的
> OpenAI 实测（能力面矩阵与「文字锁不住脸」那组对照）、§6.3 的多视图规格与
> 身份锁、§6.8 的阻塞条件。
>
> **已作废的部分**：§6.2 的后端选型、§6.2.3 的代价分析、§6.4 的
> `ZImageBackend`、§6.3「生成顺序」里「其余视图一律以主视图为参考图」这条
> 机制（改为累积锚定）、§6.5 里 `derived_from` 的单值形状（改为数组）。
>
> 重写的前置是 issue #12 待实测第 2b 项（FLUX.2 累积锁定的实际一致性）——
> 那是卡片路线唯一剩下的支点，锁不住就要重新选型。**在它出结果之前改这一章
> 是在猜。** 待办见 `docs/TODO.md`。

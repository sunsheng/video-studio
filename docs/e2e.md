...
无从判断那次覆盖了多少。`studio-cli doctor` 的 ComfyUI 那一项现在会把集群
构成一并报出来（几台、什么卡、多少显存、有没有节点失联），直接抄它即可。

**入口那一侧是多节点代理时，`waiting_on: system` 停得久是正常的。**
所有节点都到并发上限时代理会排队，而排队在我们这一侧长得像慢。判断标准不是
耗时，是 `note` 有没有在动、有没有变成带 remedy 的 `blocked_by`。见 SPEC-0017。
...
3. 同一份声明组装两次逐字节相同
4. 每份片段读得出来、元数据完整，没核验的都写了原因
5. video 通道（`clip` 锚点 / `kind: video` 参考）跑得出片，而且**下载下来的是
   成片不是锚点素材**——`LoadVideo` 会把输入回显进 history 的 `outputs`，
   只断言「有产物」的测试挡不住这一类错
6. audio 通道（锚点与参考）两条都跑得出片
...
10. **并发提交不被当成失败**：24 镜一次压进去，提交全成功、全部出片

第 8 条是整套验收里补得最晚、也最该早有的一条。AUTOGROW 槽位写成嵌套对象时
「图合法 + 跑完了 + 有产出」**三样全都成立**，参考却一个都没进模型——
只验这三样的测试对它完全无感。断言要落在「东西有没有起作用」上。

第 10 条**只验到了一半**，而且那一半要说清楚：2026-09-05 两次运行都没有真的
压出排队（最慢提交 974ms / 1411ms，代理接了就返回）。所以「排队被等过去」
只有单元测试覆盖，真机没走到。测试里写死了这个判据——压不出排队时打印警告
说明本轮没验到，不让没压满的运行冒充通过。

### 「未核验就阻塞」为什么不在这里

这条规则原来在真机这一组，靠「片段库里恰好有一个未核验的通道」才成立。三条
输入通道全核验之后它就变成了打印一行「前提不再成立」然后通过——**一条永远绿、
永远什么都不验的测试比没有更糟**，它让人以为那条规则有人守着。

现在它在 `studio_core::assembly` 的 `an_unverified_fragment_blocks_rendering`：
用合成片段库现场把 `verified` 翻成 false，**四个拼接点逐个验**（head / 参考的
输入片段 / guide 片段 / guide 的输入片段），每一处漏掉 `require_verified` 都会
红并报出是哪一处。前提永远成立，不需要 GPU——「未核验就阻塞」是纯逻辑，
本来就不该拿真机来证。

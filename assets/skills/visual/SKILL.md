...
- 每张卡写一段 identity_prompt，**一次写定**，**这张卡的**所有视图逐字复用——每个视图的 prompt 都以它开头，一个字不改，后面再接该视图特有的机位／表情。**每张卡的身份锁写的是这张卡自己那个东西**：角色卡写那个人，场景卡写那个空间，道具卡写那个道具。把角色的身份锁抄进场景卡或道具卡，出来的三张卡是同一个人——真发生过，控制面会在提交时就挡下（V13）。只有角色卡这一段要逐字包含分镜定下的身份锁。

...
- 每张卡写一段 identity_prompt，**一次写定**，**这张卡的**所有视图逐字复用——每个视图的 prompt 都以它开头，一个字不改，后面再接该视图特有的机位／取景。**那半句要写成人话，不是把视图的枚举名抄上去**——抄枚举名的结果是七个视图长得一模一样，真发生过，控制面会在提交时挡下（V14）。**而且取景要写「什么占满画面」，不要写「裁到哪」**：「双手和瓶身占据画面」有效，「肩部以上入画」无效——后者实测完全不生效，出来还是全身照。告诉模型画面里该有什么，比告诉它裁到哪可靠得多，跟「排版禁止项要写成正向约束」是同一条道理。连带一条：**特写类视图不要写「不裁切」**，它们按定义就是裁切的。**每张卡的身份锁写的是这张卡自己那个东西**：角色卡写那个人，场景卡写那个空间，道具卡写那个道具。把角色的身份锁抄进场景卡或道具卡，出来的三张卡是同一个人——真发生过，控制面会在提交时就挡下（V13）。只有角色卡这一段要逐字包含分镜定下的身份锁。
- 标出主视图（每张卡有且仅有一个）并把它排在 views 第一位——**顺序就是生成顺序**。其余视图的 derived_from 是一个列表，第一项永远是主视图。并行出八张，出来的是八个长得像但不是同一个人的角色。**后面挂什么，按构图分家**：全身类视图累积挂前面已定稿的全身类视图；特写类视图（face_close / expressions / hands_props / wardrobe_detail /场景的 detail）**只挂主视图加前面已定稿的特写类视图**，不要把全身照挂给它们。参考图会连构图一起锁——挂四到七张全身照，出来的特写就还是全身照，提示词写得再准也压不住（真发生过）。
